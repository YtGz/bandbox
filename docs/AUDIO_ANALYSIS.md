...
The interval sequence uses five levels — not just direction, but **relative magnitude**: `-2` (big drop), `-1` (small drop), `0` (flat), `+1` (small rise), `+2` (big rise). "Big" and "small" are relative to each riff's own pitch range (because the contour is normalized to unit variance before quantization). This means the intervals capture *how a riff's jumps relate to each other* — a riff that mixes small steps with one big leap looks different from one with uniform steps — but two riffs that both oscillate between just two notes will produce similar intervals regardless of the actual musical interval.

The full 200-point normalized contour (compared via DTW) captures absolute shape differences that the quantized intervals don't. The intervals serve primarily as a **fast pre-filter** — a cheap cosine similarity check that rejects obviously unrelated riff pairs before running expensive DTW on the full contour.
